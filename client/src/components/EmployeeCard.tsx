...
const statusClassMap: Record<Employee["status"], string> = {
  active: "bg-green-100 text-green-800",
  inactive: "bg-red-100 text-red-800",
  leave: "bg-yellow-100 text-yellow-800",
};

const roleClassMap: Record<Employee["role"], string> = {
    ...
    <Card className={`hover-elevate ${className ?? ""}`} data-testid={`card-employee-${employee.id}`}>
      ...
      <CardContent className="space-y-3">
        <div className="flex gap-2">
          <Badge variant="outline" className={roleClassMap[employee.role]}>
            {t(`roles.${employee.role}`)}
          </Badge>
          <Badge variant="secondary" className={statusClassMap[employee.status]}>
